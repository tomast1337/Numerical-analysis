function x = gauss_jordan_pivot(A, b)
    ...
    for i = 1:n
        ...
        % Display current state after scaling
        fprintf('Step %d: Scaling row %d\n', i, i);
        disp_augmented_matrix(A, b);
        ...
        for j = 1:n
            if j ~= i
                ...
                % Display current state after elimination
                fprintf('Step %d.%d: Eliminating row %d\n', i, j, j);
                disp_augmented_matrix(A, b);
            end
        end
    end
...
function disp_augmented_matrix(A, b)
    % Helper function to display the augmented matrix (A | b)
    augmented_matrix = [A, b];
    disp('Augmented matrix (A | b):');
    disp(augmented_matrix);
end    % GAUSS_JORDAN_PIVOT Solves linear system Ax = b using Gauss-Jordan elimination with partial pivoting
    % Input:
    %   A - coefficient matrix (n x n)
    %   b - right-hand side vector (n x 1)
    % Output:
